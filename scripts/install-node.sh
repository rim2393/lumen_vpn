...
write_node_env() {
  run mkdir -p "$(dirname "$CONFIG_FILE")"
  {
    ...
    printf 'LUMEN_SHADOWSOCKS_PLUGIN_CONFIG_FILE=%s\n' "${LUMEN_SHADOWSOCKS_PLUGIN_CONFIG_FILE:-/var/lib/lumen-node/runtime/shadowsocks-plugin/config.json}"
    printf 'LUMEN_SHADOWSOCKS_PLUGIN_LOG_FILE=%s\n' "${LUMEN_SHADOWSOCKS_PLUGIN_LOG_FILE:-/var/lib/lumen-node/runtime/shadowsocks-plugin/ssserver.log}"
    printf 'LUMEN_SHADOWSOCKS_PLUGIN_PID_FILE=%s\n' "${LUMEN_SHADOWSOCKS_PLUGIN_PID_FILE:-/var/lib/lumen-node/runtime/shadowsocks-plugin/ssserver.pid}"
    printf 'LUMEN_SHADOWSOCKS_PLUGIN_RELOAD_MODE=%s\n' "${LUMEN_SHADOWSOCKS_PLUGIN_RELOAD_MODE:-process}"
    printf 'LUMEN_NAIVE_CONFIG_FILE=%s\n' "${LUMEN_NAIVE_CONFIG_FILE:-/var/lib/lumen-node/runtime/naive/config.json}"
    printf 'LUMEN_NAIVE_LOG_FILE=%s\n' "${LUMEN_NAIVE_LOG_FILE:-/var/lib/lumen-node/runtime/naive/sing-box.log}"
    printf 'LUMEN_NAIVE_PID_FILE=%s\n' "${LUMEN_NAIVE_PID_FILE:-/var/lib/lumen-node/runtime/naive/sing-box.pid}"
    printf 'LUMEN_NAIVE_RELOAD_MODE=%s\n' "${LUMEN_NAIVE_RELOAD_MODE:-process}"
    printf 'LUMEN_OPENVPN_CONFIG_FILE=%s\n' "${LUMEN_OPENVPN_CONFIG_FILE:-/var/lib/lumen-node/runtime/openvpn/server.conf}"
    printf 'LUMEN_OPENVPN_LOG_FILE=%s\n' "${LUMEN_OPENVPN_LOG_FILE:-/var/lib/lumen-node/runtime/openvpn/openvpn.log}"
    printf 'LUMEN_OPENVPN_PID_FILE=%s\n' "${LUMEN_OPENVPN_PID_FILE:-/var/lib/lumen-node/runtime/openvpn/openvpn.pid}"
    printf 'LUMEN_OPENVPN_AUTH_SCRIPT=%s\n' "${LUMEN_OPENVPN_AUTH_SCRIPT:-/var/lib/lumen-node/runtime/openvpn/auth-user-pass.sh}"
    printf 'LUMEN_OPENVPN_USERS_FILE=%s\n' "${LUMEN_OPENVPN_USERS_FILE:-/var/lib/lumen-node/runtime/openvpn/users.txt}"
    printf 'LUMEN_OPENVPN_RELOAD_MODE=%s\n' "${LUMEN_OPENVPN_RELOAD_MODE:-process}"
